...
if settings.get("header_image"):
    header_bg_style = f"background-image: url('{settings.get('header_image')}'); background-size: cover; background-position: center;"
else:
    header_bg_style = "background: linear-gradient(90deg, #DC2626 0%, #F59E0B 100%);"

header_html = f"""
<div style='{header_bg_style} padding: 3rem; border-radius: 10px; text-align: center; margin-bottom: 2rem; min-height: 250px; display: flex; flex-direction: column; justify-content: center; align-items: center; position: relative;'>
    <div style='position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.3); border-radius: 10px;'></div>
    <div style='position: relative; z-index: 2;'>
        <h1 style='font-size: 3rem; font-weight: bold; color: {settings.get("text_color", "#FFFFFF")}; margin: 0;'>{settings.get("title", "Mon Road Trip en Italie")}</h1>
    ...
    for idx, activity in enumerate(st.session_state.activities):
        with st.container():
            ...
            if activity.get("photos"):
                ...
                # Thumbnails cliquables en dessous
                if len(activity["photos"]) > 1:
                    st.write("")  # Espace
                    thumb_cols = st.columns(len(activity["photos"]))
                    for thumb_idx, thumb_photo in enumerate(activity["photos"]):
                        with thumb_cols[thumb_idx]:
                            if st.button(f"📷 {thumb_idx + 1}", key=f"thumb_{idx}_{thumb_idx}", use_container_width=True):
                                st.session_state.carousel_index[carousel_key] = thumb_idx
                                st.rerun()
                    
                    # Afficher le compteur
                    st.markdown(f"<p style='text-align: center; color: #666; margin-top: 0.5rem;'>Photo {current_idx + 1} sur {len(activity['photos'])}</p>", unsafe_allow_html=True)
