...
.close-btn { background: none; border: none; font-size: 1.5rem; cursor: pointer; color: #DC2626; padding: 0; }
</style>
...
def create_zip_download(photos, title):
    """Crée un ZIP avec toutes les photos"""
...
if st.session_state.view == 'home':
    ...
    if st.session_state.is_admin:
        ...
        with st.form("post_form", clear_on_submit=True):
            ...
            st.subheader("📸 Photos")
            files = st.file_uploader("Photos:", accept_multiple_files=True, type=["jpg", "jpeg", "png", "gif", "webp"], key="post_photos")
...
# VUE POST
    ...
    # ENCADREMENT DE LA PUBLICATION
    st.markdown(f"""
    <div class='publication-box'>
        <div class='pub-header'>
            <div class='pub-title-date'>
    ...
    # DESCRIPTIONS DANS L'ENCADREMENT
    st.markdown("<div class='publication-box'>", unsafe_allow_html=True)
    
    if act.get("image_description"):
        st.markdown(f"""
        <div class='pub-description'>
            <p><strong>📸 Description images :</strong></p>
            <p>{act['image_description']}</p>
        </div>
        """, unsafe_allow_html=True)
